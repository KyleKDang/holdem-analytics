...
**[Live Application](https://holdem-analytics.com)** · **[Go Odds Engine](https://github.com/KyleKDang/poker-odds-engine)**
...
The Axios client is configured against the relative path `/api`, and Next.js rewrites those paths to the FastAPI origin server-side.
The rewrite list is an allowlist rather than a catch-all: only the route prefixes this app actually calls are forwarded, and anything else under `/api` returns a 404 from Next without reaching the API.
...
FastAPI generates interactive documentation from the route definitions.
With the stack running locally, Swagger UI is at `http://localhost:8000/docs` and ReDoc at `http://localhost:8000/redoc`.

The endpoints the application uses, grouped by router.
...
| Method | Endpoint | Description |
| --- | --- | --- |
| `POST` | `/hands` | Log a hand against a session |

Logged hands are read back through `/sessions/{id}/hands` rather than individually.
...
- The `/api` proxy forwards only an allowlist of route prefixes, so the deployed frontend cannot be used to reach anything the app does not call.
